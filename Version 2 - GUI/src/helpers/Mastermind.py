...
logs_namescore_path = os.path.join(version2_path, "logs", "Name_Score.txt")
...
class Master_Mind:
    def __init__(self):
        ...
        self.num_guesses = Toplevel(
            self.frame, bg="#%02x%02x%02x" % (255, 166, 0), pady=100
        )
        self.num_guesses.iconbitmap(img_path)
        ...
        self.mastermind = Message(
            self.num_guesses,
            text="Mastermind",
            font=("courier", 50),
            width=1000,
            pady=10,
            bg="#%02x%02x%02x" % (255, 166, 0),
        )
        self.mastermind.pack()
        question = Message(
            self.num_guesses,
            text="What length would you like to guess?",
            font=("courier", 20),
            width=1000,
            pady=10,
            bg="#%02x%02x%02x" % (255, 166, 0),
    ...
    def give_directions(self):
        ...
        self.directions_give = Message(
            self.directions,
            text="Pick a length you want to guess from, then guess the random order of numbers.\nThe numbers may repeat and they go from 1 to 6.",
            width=1000,
            pady=200,
            font=("courier", 20),
        )
    ...
    def win_lose(self):
        if ("|%s|" % ("".join(self.game))) == ("".join(self.board)):
            ...
            printword2 = Message(
                self.gameboard,
                text="The correct sequence was %s" % ("".join(self.board)),
                width=1000,
                bg="#%02x%02x%02x" % (255, 166, 0),
                font=("courier", 20),
            )
            ...
            printword = Message(
                self.gameboard,
                text="The correct sequence was %s" % ("".join(self.game)),
                width=1000,
                bg="#%02x%02x%02x" % (255, 166, 0),
                font=("courier", 20),
            )
    ...
    def get_guess(self):
        if self.count != 20:
            ...
            self.digits = Message(
                self.gameboard,
                text="Please enter your guess of %s digits." % (self.num.get()),
                width=1000,
                bg="#%02x%02x%02x" % (255, 166, 0),
                font=("courier", 20),
            )
    ...
    def process(self, n):
        try:
            ...
            messagebox.showwarning(
                "Invalid Entry", "Number must be in the correct range."
            )
        except ValueError:
            self.get_guess()
        ...
        if self.board != ["|", "|"]:
            if self.count < 10:
                ...
                self.score = Message(
                    self.gameboard,
                    text="Exactly: %s\tClose: %s"
                    % (self.exact_count, self.close_count),
                    width=1000,
                    bg="#%02x%02x%02x" % (255, 166, 0),
                    font=("courier", 10),
                )
    ...
    def Record_Score(self):
        self.highScores = Toplevel(
            self.gameboard, width=1000, bg="#%02x%02x%02x" % (255, 166, 0)
        )
        self.score = self.count + 1
        self.askname = Message(
            self.highScores,
        ...
        self.askname.pack()
        file1 = open(logs_namescore_path, "r")
    ...
    def addscore(self, n):
        ...
        file2 = open(logs_namescore_path, "w")
    ...
    def print_score(self):
        self.yourscore = Message(
            self.highScores,
            text="Your Score:  " + str(self.count + 1),
            bg="#%02x%02x%02x" % (255, 166, 0),
        )
        self.yourscore.pack()
        # print("\t\t\t\t\t\t\t\t\t\t\tHIGH SCORES:\tLENGTH:\n\n")
        file = open(logs_namescore_path, "r")
        text = file.readlines()
        i = 0
        for line in text:
            if i <= 10:
                self.highscores1 = Message(
                    self.highScores,
                    text=line,
                    width=1000,
                    bg="#%02x%02x%02x" % (255, 166, 0),
            ...
            i += 1
        i = 0
        self.play_againbutton = Button(
            self.highScores, text="Continue", width=40, command=lambda: self.function()
        )
