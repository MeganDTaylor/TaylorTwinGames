...
logs_highscore_path = os.path.join(version2_path, "logs", "High_Score.txt")
...
class GuessingGame:
    ...
    def give_directions(self):
        ...
        self.directions_give = Message(
            self.directions,
            text="You have as many tries as you need to guess the correct number. It lies between 1 and 100.",
            width=1000,
            pady=200,
    ...
    def play_game(self):
        ...
        self.best_score = Message(
            self.frame,
            text="Best Score: %s" % (self.high_score),
    ...
    def check(self, n):
        ...
        if int(self.random_number) == int(self.guess_number):
            ...
            self.correct = Message(
                self.frame,
                text="That is correct! Your score is %s" % (self.turn),
                width=400,
                font=("century gothic", 20),
                bg="#%02x%02x%02x" % (255, 0, 0),
            )
        ...
        self.progress = Message(
            self.frame,
            text="%s   Guess: %s   Status: %s"
            % (self.turn, self.guess_number, self.status),
            width=400,
            font=("century gothic", 15),
            bg="#%02x%02x%02x" % (255, 0, 0),
        )
